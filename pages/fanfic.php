...
$page_title = 'Fanfiction';
require_once dirname(__DIR__) . '/includes/header.php';

// Categories from write-story.php
...
// Combine all for filtering purposes
...
// Base query - filter fanfictions with library count
...
WHERE (s.is_fanfiction = 1 OR s.content_type = 'fanfic' OR LOWER(s.tags) LIKE '%fanfic%' OR LOWER(s.tags) LIKE '%fanfiction%')";
$params = [];
...
// Get category counts for fanfictions only
$categoryCounts = [];
try {
    $countStmt = $pdo->prepare("SELECT COUNT(*) as count FROM stories WHERE (is_fanfiction = 1 OR content_type = 'fanfic' OR LOWER(tags) LIKE '%fanfic%' OR LOWER(tags) LIKE '%fanfiction%') AND (tags LIKE ? OR genre LIKE ?)");
    foreach ($categories as $cat) {
        $countStmt->execute(["%$cat%", "%$cat%"]);
...
<div class="max-w-7xl mx-auto px-4 py-8">
    <div class="mb-8">
        <h1 class="text-4xl font-bold text-emerald-700 dark:text-emerald-400 mb-2">✍️ Fanfiction</h1>
        <p class="text-gray-600 dark:text-gray-400">Discover fan-created stories from our talented community</p>
    </div>
        ...
        <div class="w-72 flex-shrink-0">
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow border border-emerald-200 dark:border-emerald-900 sticky top-20">
                ...
                <form method="get" class="mb-6">
                    ...
                    <input type="text" name="search" placeholder="Search fanfiction..." value="<?= htmlspecialchars($searchQuery ?? '') ?>"
                ...
                <div class="space-y-1 max-h-[30vh] overflow-y-auto mb-6 pb-4 border-b border-gray-300 dark:border-gray-700">
                    ...
                    <?php foreach ($genres as $gen): 
                        $genParams = array_filter(['category' => $gen, 'search' => $searchQuery]);
                ...
                <div class="space-y-1 max-h-[50vh] overflow-y-auto">
                    <?php foreach ($tags as $tag): 
                        $tagParams = array_filter(['category' => $tag, 'search' => $searchQuery]);
        ...
        <div class="flex-1">
            <?php if (empty($books)): ?>
                <div class="p-8 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-center border border-blue-200 dark:border-blue-800">
                    <p class="text-lg text-gray-700 dark:text-gray-300 font-semibold">📭 No fanfiction found</p>
