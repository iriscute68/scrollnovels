...
// Base query - filter only webtoons
$query = "SELECT id, title, slug, cover, cover_image, synopsis, genre, tags, author_id, views, likes, status, content_type, is_fanfiction FROM stories WHERE content_type = 'webtoon'";
$params = [];
...
$query .= " ORDER BY views DESC, created_at DESC LIMIT 100";
...
// Get category counts for webtoons only
$categoryCounts = [];
try {
    $countStmt = $pdo->prepare("SELECT COUNT(*) as count FROM stories WHERE content_type = 'webtoon' AND (tags LIKE ? OR genre LIKE ?)");
...
<div class="max-w-7xl mx-auto px-4 py-8">
    <div class="mb-8">
        <h1 class="text-4xl font-bold text-emerald-700 dark:text-emerald-400 mb-2">🎨 Webtoons</h1>
        <p class="text-gray-600 dark:text-gray-400">Discover amazing visual stories from our community</p>
    </div>
        ...
        <div class="w-72 flex-shrink-0">
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow border border-emerald-200 dark:border-emerald-900 sticky top-20">
                ...
                <form method="get" class="mb-6">
                    ...
                    <input type="text" name="search" placeholder="Search webtoons..." value="<?= htmlspecialchars($searchQuery ?? '') ?>"
        ...
        <div class="flex-1">
            <?php if (empty($books)): ?>
                <div class="p-8 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-center border border-blue-200 dark:border-blue-800">
                    <p class="text-lg text-gray-700 dark:text-gray-300 font-semibold">📭 No webtoons found</p>
