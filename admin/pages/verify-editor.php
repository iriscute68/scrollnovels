<?php
// admin/pages/verify-editor.php - Manage editor verification requests

$pending = $pdo->query("
    SELECT vr.*, u.username, u.email, u.profile_image, u.role
    FROM verification_requests vr
    JOIN users u ON vr.user_id = u.id
    WHERE u.role = 'editor' AND vr.status = 'pending'
...
$approved = $pdo->query("
    ...
    FROM verification_requests vr
    JOIN users u ON vr.user_id = u.id
    WHERE u.role = 'editor' AND vr.status = 'approved'
    ORDER BY vr.created_at DESC
    LIMIT 20
")->fetchAll();
?>

<h4>✏️ Editor Verification</h4>
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <?php if (empty($pending)): ?>
            <div class="alert alert-info">No pending editor applications</div>
        <?php else: ?>
            <div class="row">
                        ...
                        <div class="card border-warning">
                            <div class="card-body">
                                ...
                                <div class="btn-group w-100" role="group">
                                    <button onclick="approveEditor(<?= $req['user_id'] ?>, '<?= htmlspecialchars($req['username']) ?>')" class="btn btn-sm btn-success">
                                        <i class="fas fa-check"></i> Approve
                                    </button>
                                    <button onclick="rejectEditor(<?= $req['id'] ?>)" class="btn btn-sm btn-danger">
...
<div class="card">
    <div class="card-header bg-success text-white">
        <h5 class="mb-0">✅ Approved Editors (<?= count($approved) ?>)</h5>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-sm">
                ...
                <tbody>
                    <?php foreach ($approved as $editor): ?>
                        <tr>
                            <td><?= htmlspecialchars($editor['username']) ?></td>
                            <td><?= htmlspecialchars($editor['email']) ?></td>
                            <td><?= htmlspecialchars($editor['specialties'] ?? 'N/A') ?></td>
                            <td>$<?= $editor['service_rate'] ?? 'N/A' ?></td>
                            <td><?= date('M d, Y', strtotime($editor['created_at'])) ?></td>
                            <td>
                                <button class="btn btn-sm btn-outline-danger" onclick="revokeEditor(<?= $editor['user_id'] ?>)">
...
function approveEditor(userId, username) {
    if (!confirm('Approve ' + username + ' as an editor?')) return;
    
    fetch('<?= site_url('/api/admin/approve-editor.php') ?>', {
        method: 'POST',
...
function rejectEditor(requestId) {
    if (!confirm('Reject this editor application?')) return;
...
function revokeEditor(userId) {
    if (!confirm('Revoke editor status from this user?')) return;
    
    fetch('<?= site_url('/api/admin/revoke-editor.php') ?>', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: userId })
    })
